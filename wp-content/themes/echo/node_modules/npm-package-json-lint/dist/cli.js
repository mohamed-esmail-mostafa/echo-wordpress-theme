...
`,{flags:{quiet:{type:`boolean`,alias:`q`,default:!1},noConfigFiles:{type:`boolean`,alias:`ncf`,default:!1},configFile:{type:`string`,alias:`c`,default:``},ignorePath:{type:`string`,alias:`i`,default:``},maxWarnings:{type:`number`,alias:`mw`,default:1e7}}});let qt=Kt;Q(`patterns: ${qt}`),qt.length===0&&(Q(`No lint targets provided, defaulting to "."`),console.log(l.default.red.bold(`No lint targets provided, defaulting to the current working directory.`)),qt=[`.`]);try{let e=Gt.zeroClean;Q(`Creating NpmPackageJsonLint instance`);let t=new Bt({cwd:process.cwd(),configFile:$.configFile,patterns:qt,ignorePath:$.ignorePath,quiet:$.quiet}).lint();Q(`NpmPackageJsonLint.lint complete`),Q(`Reporter.write starting`),Wt(t,$.quiet),Q(`Reporter.write complete`),t.warningCount>$.maxWarnings&&(Q(`Max warnings exceeded`),e=Gt.exceedMaxWarnings),t.errorCount>0&&(Q(`Lint errors detected`),e=Gt.twoLintErrorsDetected),process.exit(e)}catch(e){console.log(l.default.red.bold(e.message)),process.exit(Gt.runTimeException)}
